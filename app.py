...
def index():
    ...
    if request.method == "GET":
        ...
        return render_template("index.html", source=full_filename, destination=full_filename, result=full_filename, error=None)

    # Execute if request is POST
    if request.method == "POST":
        # Validate file uploads
        if 'source_image' not in request.files or 'destination_image' not in request.files:
            return render_template('index.html', source='images/white_bg.jpg', destination='images/white_bg.jpg', 
                                 result='images/white_bg.jpg', error="Both source and destination images are required")

        source_upload = request.files['source_image']
        destination_upload = request.files['destination_image']

        if source_upload.filename == '' or destination_upload.filename == '':
            return render_template('index.html', source='images/white_bg.jpg', destination='images/white_bg.jpg', 
                                 result='images/white_bg.jpg', error="Please select both source and destination images")

        # Generating unique names to save images
        ...
        try:
            ...
            if not faces:
                return render_template('index.html', source=sfull_filename, destination=dfull_filename, 
                                     result='images/white_bg.jpg', error="No face detected in source image")
            ...
            if not faces2:
                return render_template('index.html', source=sfull_filename, destination=dfull_filename, 
                                     result='images/white_bg.jpg', error="No face detected in destination image")
            ...
            return render_template('index.html', source=sfull_filename, destination=dfull_filename, result=rfull_filename, error=None)

        except Exception as e:
            return render_template('index.html', source='images/white_bg.jpg', destination='images/white_bg.jpg', 
                                 result='images/white_bg.jpg', error=f"Error processing images: {str(e)}")
